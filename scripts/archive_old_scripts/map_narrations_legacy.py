...
KEYWORDS = {
    'Locomotion': [
        'walk', 'run', 'step', 'move', 'enter', 'leave', 'climb', 
        'stand up', 'sit down', 'go to', 'approach', 'descend', 'ascend',
        'navigate', 'journey'
    ],
    'Manual Work': [
        'cut', 'saw', 'hammer', 'drill', 'sand', 'wipe', 'scrub', 'wash', 'clean',
        'mix', 'stir', 'pour', 'shake', 'rub', 'dig', 'push', 'pull', 
        'lift', 'throw', 'catch', 'carry', 'hold', 'grab', 'take', 'put', 'place',
        'pick', 'drop', 'squeeze', 'roll', 'fold', 'stick', 'spray', 'click', 'cover',
    ...
    'Stationary': [
        ...
        'sit', 'stand', 'kneel', 'crouch', 'converse'
    ]
}
...
def map_text_to_label(text):
    ...
    # 4. Stationary (Explicit only)
    if any(k in text for k in KEYWORDS['Stationary']):
        return 'Stationary'
...
def main():
    ...
    for video_uid, video_data in tqdm(narrations_data.items()):
        ...
        for entry in narrations:
            ...
            if label is not None: # Only add if a label was successfully assigned
                labeled_data.append({
                    'video_uid': video_uid,
