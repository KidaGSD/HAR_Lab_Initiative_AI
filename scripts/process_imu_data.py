...
def make_windows(aligned_data, window_sec=1.0, hop_sec=1.0, fps=50):
    ...
    # aligned_data['timestamp'] is the time array
    timestamps = aligned_data['timestamp']
    
    for i in range(n_windows):
        start = i * hop_len
        end = start + window_len
        imu_windows.append(imu[start:end])
        ts_windows.append(timestamps[start:end])
        if gaze is not None:
    ...
    for uid in tqdm(uids):
        ...
        if aligned:
            ...
            if windows:
                ...
                # Keys expected by EgoExoDataset: 'gaze', 'traj' (IMU), 'timestamp'
                # windows['imu'] is (N, 50, 6)
                # We need a timestamp for each window (e.g., center or start). 
                # Let's save the full timestamp array for the window to be precise.
                # windows['timestamp'] should be (N, 50)
                np.savez(uid_dir / "seq.npz", gaze=windows['gaze'], traj=windows['imu'], timestamp=windows['timestamp'])
